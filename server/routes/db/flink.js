// Flink 路由(db.js 拆分,行为零变化):
//  - 交互查询 /flink/query、异步通道 /flink/async、流式任务管理 /flink/jobs
//  - 连接器与 DDL 生成 /flink/connectors /flink/ddl/generate
//  - PreJob(yarn-per-job)/flink/prejob/*
// 安全策略:所有 flink 类型任务(交互查询/流式任务/PreJob 提交)一律要求 checkFlinkAccess
// 矩阵授权;元数据类接口(连接器/DDL/任务列表/日志)为只读,不强制。
...
export function setupFlinkRoutes(app) {

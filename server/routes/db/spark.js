// Spark 路由(db.js 拆分,行为零变化):
//  - /api/spark/auth 写解锁(密码换 token,12h,绑定签发用户)
//  - /api/spark/query 同步执行(sql/pyspark)、/jobs 异步任务、logs/status/stages/config/cancel
// 权限:checkSparkAccess 矩阵 v2;写操作透传 writeUnlocked=true + X-Spark-Write 共享密钥(db-proxy 物理校验)。
...
import { isSparkWriteSql, issueSparkToken, gcSparkTokens, SPARK_WRITE_TOKEN_TTL } from './shared.js'

export function setupSparkRoutes(app) {

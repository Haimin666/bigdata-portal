<template>
  <ion-page>
    ...
    <ion-content :fullscreen="true"><main class="page-shell offline-shell">
      <button class="full-dev-link" type="button" @click="webOpen = true"><span>打开完整开发平台</span><small>复杂 DAG 配置</small><ion-icon :icon="expandOutline" /></button>
      <div class="offline-sticky-controls">
      ...
      <template v-if="view === 'projects'">
        <div class="project-search"><ion-searchbar v-model="keyword" class="standalone-search" placeholder="搜索项目" :debounce="150" /></div>
        <div class="result-meta"><span>可访问项目 · {{ filteredProjects.length }} 个</span><span v-if="loading">同步中…</span><button v-else-if="keyword" class="clear-filter" type="button" @click="keyword = ''">清除筛选</button></div>
      ...
      <template v-else-if="view === 'workflows'">
        <div class="result-meta"><span>工作流 · {{ filteredWorkflows.length }} 条</span><span v-if="loading">同步中…</span><button v-else-if="keyword" class="clear-filter" type="button" @click="keyword = ''">清除筛选</button></div>
        <section class="card-stack">
          <article v-for="item in filteredWorkflows" :key="item.id" class="ops-card workflow-card" :class="{ expanded: isWorkflowExpanded(item) }">
            ...
            <div class="workflow-actions">
              ...
              <button v-if="canOperate" type="button" :class="{ 'danger-action': isOnline(item.releaseState) }" :disabled="isBusy('workflow', item.id)" @click="confirmWorkflowRelease(item)"><small>工作流</small><strong>{{ isOnline(item.releaseState) ? '下线' : '上线' }}</strong></button>
              <button v-if="canOperate" type="button" :disabled="isBusy('workflow', item.id) || !isOnline(item.releaseState)" @click="confirmStart(item)"><small>手动</small><strong>启动一次</strong></button>
              <button v-if="canOperate" type="button" :disabled="isWorkflowScheduleBusy(item) || workflowScheduleState(item).loading" @click="openScheduleActions(item)"><small>定时</small><strong>{{ workflowScheduleLabel(item) }}</strong></button>
            </div>
                ...
                <article v-for="instance in workflowTaskState(item).instances" :key="instance.id" class="workflow-instance">
                  ...
                  <div class="instance-actions">
                    ...
                    <template v-if="canOperate">
                      <button v-if="canStop(instance)" type="button" class="danger-action" :disabled="isBusy('instance', instance.id)" @click="confirmWorkflowInstance(item, instance, 'STOP', '停止')">停止</button>
                      <button type="button" :disabled="isBusy('instance', instance.id)" @click="confirmWorkflowInstance(item, instance, 'REPEAT_RUNNING', '重跑')">重跑</button>
                    </template>
                  </div>
                </article>
...
const keyword = ref(''); const total = ref(0); const loading = ref(false); const busyKeys = ref<string[]>([]); const error = ref('')
...
function operationKey(scope: string, id: string | number) { return `${scope}:${id}` }
function isBusy(scope: string, id: string | number) { return busyKeys.value.includes(operationKey(scope, id)) }
function isWorkflowScheduleBusy(item: DsProcessDefinition) { return workflowScheduleState(item).items.some((schedule) => isBusy('schedule', schedule.id)) }
...
async function confirmWrite(header: string, message: string, destructive: boolean, action: () => Promise<unknown>, refresh: () => Promise<unknown>, key: string) { const alert = await alertController.create({ header, message, buttons: [{ text: '取消', role: 'cancel' }, { text: '确认执行', role: destructive ? 'destructive' : undefined, handler: () => runWrite(action, refresh, key) }] }); await alert.present() }
async function runWrite(action: () => Promise<unknown>, refresh: () => Promise<unknown>, key: string) { if (busyKeys.value.includes(key)) { (await toastController.create({ message: '该目标操作正在进行，请稍后重试', duration: 1400, position: 'bottom' })).present(); return } busyKeys.value = [...busyKeys.value, key]; error.value = ''; try { await action(); (await toastController.create({ message: '操作成功', duration: 1400, position: 'bottom' })).present(); await refresh() } catch (cause) { setError(cause, '操作失败') } finally { busyKeys.value = busyKeys.value.filter((item) => item !== key) } }
function confirmWorkflowRelease(item: DsProcessDefinition) { const sourceProject = item._projectName || project.value; const online = !isOnline(item.releaseState); return confirmWrite(`${online ? '上线' : '下线'}工作流？`, `${sourceProject} / ${item.name}（#${item.id}）`, !online, () => releaseWorkflow(sourceProject, item.id, online), reload, operationKey('workflow', item.id)) }
function confirmStart(item: DsProcessDefinition) { const sourceProject = item._projectName || project.value; return confirmWrite('启动工作流？', `${sourceProject} / ${item.name}（#${item.id}），失败策略 CONTINUE。`, false, () => startWorkflow(sourceProject, item.id), reload, operationKey('workflow', item.id)) }
function confirmSchedule(item: DsSchedule) { const sourceProject = detailProject; const online = !isOnline(item.releaseState); return confirmWrite(`${online ? '上线' : '下线'}定时？`, `${workflowDetail.value?.name || '工作流'} / ${item.crontab || `定时 #${item.id}`}`, !online, () => setScheduleState(sourceProject, item.id, online), async () => { if (workflowDetail.value) { await openWorkflow(workflowDetail.value); await refreshWorkflowSchedules(workflowDetail.value) } }, operationKey('schedule', item.id)) }
function confirmWorkflowInstance(workflow: DsProcessDefinition, item: DsProcessInstance, type: DsExecuteType, label: string) { const sourceProject = item._projectName || workflow._projectName || project.value; return confirmWrite(`${label}工作流实例？`, `${item.name} / 实例 #${item.id}，请确认目标无误。`, type === 'STOP', () => executeProcess(sourceProject, item.id, type), () => loadWorkflowTasks(workflow, true), operationKey('instance', item.id)) }

<template>
  <ion-page>
    <ion-header class="ion-no-border"><ion-toolbar><ion-title>YARN 应用</ion-title><ion-buttons slot="end"><ion-button @click="load"><ion-icon :icon="refreshOutline" /></ion-button></ion-buttons></ion-toolbar></ion-header>
    <ion-content :fullscreen="true">
      <main class="page-shell">
        ...
        <div class="result-meta"><span>{{ filteredApps.length }} 个应用</span><span v-if="loading">同步中…</span></div>
        <section class="card-stack">
          <article v-for="app in filteredApps" :key="app.id" class="ops-card">
            ...
            <div class="card-actions"><ion-button fill="clear" size="small" @click="copy(app.id)">复制 ID</ion-button><ion-button v-if="canKill(app)" color="danger" fill="clear" size="small" @click="confirmKill(app)">终止应用</ion-button></div>
          </article>
          <div v-if="!loading && !filteredApps.length" class="empty-state"><ion-icon :icon="checkmarkCircleOutline" /><strong>当前没有匹配应用</strong><span>尝试切换状态或搜索条件</span></div>
        </section>
...
import { alertController, toastController, IonButton, IonButtons, IonContent, IonHeader, IonIcon, IonPage, IonSearchbar, IonSelect, IonSelectOption, IonTitle, IonToolbar } from '@ionic/vue'
...
async function doKill(app: YarnApp) { try { await killApp(selectedRm.value, app.id); await load() } catch (cause) { error.value = cause instanceof Error ? cause.message : '终止失败' } }
onMounted(load)
</script>

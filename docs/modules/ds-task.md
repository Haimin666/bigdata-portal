...
DolphinScheduler 工作流与实例监控:项目/工作流/定义详情/调度的只读查询，搜索/筛选、状态色、**单页分栏**(左=当前实例列表,右=跨项目依赖树)、级联重跑和受控写操作。桌面监控默认看**当天(00:00→现在)**；移动端工作流实例与任务实例默认看**近两天**。
...
| 视图 | `src/views/ds/DsDepsPanel.vue` | **右栏依赖 DAG 面板**(内嵌,替代原弹窗):G6.Graph(dagre) 有向图画上游→当前→下游,浅色卡片节点标最近一次执行状态/执行耗时;勾选级联调起 + 节点右键菜单(复制名称/执行当前及以后)+ hover 启停时间;由主视图异步加载,只在打开依赖侧栏时下载 |
...
- **状态色**(与 yarn 对齐):运行中/已提交 = Vben 品牌蓝 `#006be6`;成功 = 绿;失败 = 红;暂停 = 橙;停止/已终止 = 灰(`StatusBadge`)
...
- `/dolphinscheduler/projects/:project/{process/release,executors/start-process-instance,schedule/online,schedule/offline,executors/execute}` 由网关执行门禁保护：viewer 一律禁止，dev/admin 还需 `dsTask` 模块权限；桌面端和移动端不得绕过网关直连海豚。

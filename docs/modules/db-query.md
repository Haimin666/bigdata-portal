...
- **结果表格**:斑马纹 + 固定序号列 + 表头按数值列右对齐;列名点击排序,悬停 ⧉ 按钮复制列名(复制按钮绝对定位在排序箭头左侧,用 padding 预留,不与排序重叠)。单元格点击复制,NULL 低饱和胶囊。结果区合成一个整体卡片(.result-card):**表格 → 翻页(紧贴数据下方) → 底部信息条(行×列/复制/选择模式)**,内部细线分隔

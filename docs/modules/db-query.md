...
| 层 | 文件 | 说明 |
|---|---|---|
| 视图 | `src/views/db/QueryView.vue` | 主工作台:引擎/数据源下拉、Monaco 编辑器(SqlEditor 封装)、快捷键(Ctrl+Enter 执行/Ctrl+Shift+F 格式化)、结果多 tab、执行历史、日志面板;支持双击表目录表节点生成预览查询自动执行;结果表**双击单元格行内编辑**(仅带主键的单表预览)生成 UPDATE 写前确认;工具栏 **EXPLAIN** 打开执行计划树;编辑器 **schema 驱动补全**(Ctrl+Space);格式化走 **sql-formatter**(方言 mysql/oracle/spark,${var} 占位保护,失败降级自研) |
...
| 视图 | `src/views/db/SqlTreePanel.vue` | 脚本文件树(本地 scripts 存储,右键新建/重命名/删除) + 表目录(**引擎 → 库 → 表 → 字段**:mysql/oracle 数据源分组展示 + Spark 组(经新 `/spark/schema/*` 元数据,SHOW/DESC 只读+db-proxy 侧 5 分钟 TTL 缓存,Spark 可用性由 spark/status 探测),含表/字段注释,双击表预览,复制表名(Spark 带 db. 前缀)/建表语句) + **历史/收藏**(localStorage 持久化,点击回填编辑器执行) |

...
- **视觉层级**:页面标题、筛选工具栏、资源概览和应用列表按四层纵向组织;集群总览为四张独立轻量指标卡;应用表格使用弱化的横向行分隔而非密集竖向网格线。≤760px 时筛选控件两列折行，RM 与搜索跨满行，偏好操作可横向滚动。桌面端主页面统一使用蓝灰主题面板、层级化指标卡和明确的列表容器边界,不改变表格内容尺寸。
- **标题策略**:主页面不重复展示模块标题,侧栏与 Tab 负责导航标识,将操作提示放在筛选器、状态卡和空状态中。
- **滚动策略**:页面根节点不滚动;队列明细、应用表格视图分别由自身 body 内滚动,卡片视图由卡片网格内滚动,分页和筛选工具栏固定在内容区。

...
# 用 BASE_IMAGE 指向内部 registry,如 harbor.example.com/library/node:22-alpine。
# Capacitor CLI 8 要求 Node >=22,桌面端与移动端统一在同一构建镜像中完成。
...
COPY server ./server
ENV NODE_ENV=production PORT=9910
EXPOSE 9910
